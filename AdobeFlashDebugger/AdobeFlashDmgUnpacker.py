...
class AdobeFlashDmgUnpacker(Processor):
    ...
    def decompress_plugin(self, finalize_script_path, pkgroot):
        ...
        try:
            ...
            cache_path = os.path.join('/Users/vmule/Library/AutoPkg/Cache',
                'com.github.autopkg.munki.FlashDebuggerExtractPackage')
            unpkg_path = os.path.join(cache_path,
                'Payload/AdobeFlashPlayerComponent.pkg')

            os.remove(os.path.join(unpkg_path, 'Payload'))
            ...
            pkg_size = str(int(subprocess.check_output(['du','-sk', pkgroot]).split()[0]) - 4)
        ...
        cmd = ('pkgutil', '--flatten', unpkg_path, pkg_path)
        subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
